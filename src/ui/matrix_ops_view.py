...
class MatrixOpsView(ctk.CTkFrame):
    ...
    def _setup_tab_ops(self):
        ...
        self.entry_ops_escalar = ctk.CTkEntry(esc_row, width=60, justify="center")
        self.entry_ops_escalar.pack(side="left", padx=6)
        self.entry_ops_escalar.insert(0, "2")

        # Scroll con cuadrículas de A y B
        self.scroll_ops = ctk.CTkScrollableFrame(left, height=240)
        ...
        ctk.CTkButton(op_grid, text="A − B", command=lambda: self._calc_op("resta"),
                      fg_color=("#0284c7", "#0369a1")).grid(row=0, column=1, padx=3, pady=3, sticky="ew")
        ctk.CTkButton(op_grid, text="c · A", command=lambda: self._calc_op("esc_A"),
                      fg_color=("#0d9488", "#0f766e")).grid(row=1, column=0, padx=3, pady=3, sticky="ew")
        ctk.CTkButton(op_grid, text="c · B", command=lambda: self._calc_op("esc_B"),
                      fg_color=("#0d9488", "#0f766e")).grid(row=1, column=1, padx=3, pady=3, sticky="ew")
        ctk.CTkButton(op_grid, text="A × B  (Producto Matricial)", command=lambda: self._calc_op("prod"),
                      fg_color=("#7c3aed", "#6d28d9"), font=ctk.CTkFont(size=12, weight="bold")
    ...
    def _cargar_ejemplo_ops(self):
        ejemplos = [
            ...
            # Slide 7 Ej: A (3x3) x B (3x3)
            {
                ...
                "A": [[2, 3, 4], [1, -2, 0], [3, 1, -1]],
                "B": [[1, 0, 2], [3, -1, 0], [0, 4, 1]],
                "c": -1, "desc": "Producto Matricial A(3×3) × B(3×3)"
            },
            # Dimensiones incompatibles para mostrar error
            {
                "mA": 2, "nA": 3, "mB": 2, "nB": 2,
                "A": [[1, 2, 3], [4, 5, 6]],
                "B": [[1, 0], [0, 1]],
                "c": 5, "desc": "⚠️ A(2×3) × B(2×2) — Dimensiones incompatibles para multiplicación"
            }
        ]
        ...
        self._generar_matrices_ops()
        self.entry_ops_escalar.delete(0, "end"); self.entry_ops_escalar.insert(0, str(ej["c"]))
    ...
    def _calc_op(self, op: str):
        ...
        try:
            ...
            c_str = self.entry_ops_escalar.get().strip()
        ...
        try:
            if op == "suma":
                ...
                lineas.append(f"Condición: Ambas matrices deben ser {mA}×{nA}.")
                ...
                lineas.append(f">> PRODUCTO ESCALAR: {c_fmt} · A")
                lineas.append(f"\nEscalar c = {c_fmt}")
                lineas.append(f"A ({mA}×{nA}):\n{self._fmt_mat(A, modo)}")
                lineas.append("Procedimiento: (c·A)_ij = c · A_ij para cada posición.")
                lineas.append(f"\nResultado {c_fmt}·A ({mA}×{nA}):\n{self._fmt_mat(R, modo)}")

            elif op == "esc_B":
                R = multiplicar_matriz_escalar(c, B)
                lineas.append(f">> PRODUCTO ESCALAR: {c_fmt} · B")
                lineas.append(f"\nEscalar c = {c_fmt}")
                lineas.append(f"B ({mB}×{nB}):\n{self._fmt_mat(B, modo)}")
                lineas.append("Procedimiento: (c·B)_ij = c · B_ij para cada posición.")
                lineas.append(f"\nResultado {c_fmt}·B ({mB}×{nB}):\n{self._fmt_mat(R, modo)}")
